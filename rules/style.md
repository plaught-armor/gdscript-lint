...
- **Prefer `Packed*Array` over `Array[primitive]`** (S6) — element type has packed variant (`int`/`float`/`String`/`Vector2`/`Vector3`/`Color`/byte) → use it: faster indexed access, no per-element Variant boxing. Fall back to `Array[int]`/`Array[float]` only when genuinely need Variant elements (mixed types, Object refs). Declare `var`/`static var`, never `const` — a `const Packed*Array` is not read-only and an append through any binding rewrites the constant process-wide, where `const Array[T]` raises (C1); a shared table you actually want frozen is the one case to keep `Array[T]`; init bare literal — `var x: PackedInt32Array = [1, 2, 3]` or `= []`, never `PackedInt32Array([...])` / `PackedInt32Array()` constructor (S6b; typed annotation does conversion). On `@export` field constructor form not just redundant — reads back null in inspector, persists empty on save/reimport, silently drops authored data ([#106965](https://github.com/godotengine/godot/issues/106965)); fix = bare literal, **not** downgrade to `Array[int]`. **Two flags, split by stake:** S6b = plain field/local, wrapper merely redundant (STYLE); **S6c** = same wrapper on an `@export` field, data loss (CORRECT-tier, fix it). Both fire only on typed assignment `: Packed*Array = Packed*Array(`, where `[]` provably converts — arg-position `foo(PackedByteArray())` left alone (there `[]` = plain `Array`). See [`engine-bugs.md`](engine-bugs.md) C1.
...
- **No per-use `as` inside an `is` guard** (H14) — **`is` does not narrow.** Measured 4.8.dev with `unsafe_method_access=2`: inside `if x is T:` the analyzer still calls `x` `Variant` ("The method `keys()` is not present on the inferred type `Variant`"), and the same holds for a class-typed var (`if n is Node2D:` leaves `n` a `Node`). The narrowing you see in the editor comes from `modules/gdscript/gdscript_editor.cpp:2488`, an **autocomplete-only** hack the source itself labels "Super dirty hack, but very useful" — the type checker and the compiler have no such pass. So `(x as T).member` is not a *redundant* cast, it is a **repeated** one: every use pays the runtime check again, and the bare `x.member` it replaces is an unsafe dynamic access (`unsafe_property_access` fires on it). **Bind a typed local once, hoisted above the loop where the guard allows it** — that is the only form the checker types. Measured ns/access (`bench_redundant_cast.gd`, 2M iterations, best of 7): hoisted bind **23.0**, bare `x.member` **44.7**, per-use `(x as T).member` **61.2**, bind repeated per iteration **61.7**. Order: hoisted typed bind > bare access > per-use cast.
- **Typed-container access already typed** (H14b) — `Dictionary[K, V].get(k)` / `dict[k]` / `Array[T][i]` return typed `V` / `T`. Recasting with `as T` repeats a runtime check for a type the container already carries **statically** — unlike H14, the narrowing here is real (verified: `var s: String = d[0].x` on a `Dictionary[int, Foo]` errors "Cannot assign a value of type int"), so the cast is genuinely redundant, not merely repeated. `var cs: CellState = _cells.get(path)` — done. ⊥ `... as CellState`. Containers carry type; trust them.
