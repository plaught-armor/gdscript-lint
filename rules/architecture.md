...
Every Godot project ships these. **Autoloads** need state (signals, mutable cache) or `_ready`-driven boot logic. **Static-only RefCounted globals** (no autoload entry) accessed by `class_name`, pay no autoload cost.
...
| `RegistryRoot` | `Node` | Yes | Boot-references each domain registry (`ItemRegistry`, `EnemyRegistry`, ...) in `_ready` → force `_static_init` early + in known order. Registries self-validate inside `_static_init`. |

Promotion path: static-only RefCounted → autoload Node only when state genuinely needed (cache, RNG seed, signals). See [`dod.md`](dod.md) D9. WorldConstants = canonical static-only exemplar — `extends RefCounted`, lives in `scripts/systems/`, no `[autoload]` entry.

**Autoload scripts must NOT declare `class_name`.** Autoload's registered name (`[autoload]` key in `project.godot`) already global identifier; matching `class_name` collides — Godot errors `Class 'Foo' hides an autoload singleton`. So `SoundBus`/`SaveSystem`/`RegistryRoot` bare `extends Node`, accessed by autoload name (`SaveSystem.write_slot(...)`). Trade-off: autoload name **not** usable as type annotation (`var s: SaveSystem` fails) — fine for singleton. Need the type too → give `class_name` *different* spelling from autoload key (e.g. `class_name SaveSystemNode` + autoload `SaveSystem`); canonical default no `class_name`. `class_name` stays only on **non-autoloaded** exemplars: `WorldConstants` (static-only RefCounted), `*Registry` RefCounted tables, `Def`/`Record`/`System` classes.
...
`enum Id` lives inside system owning the concept (`ItemRegistry.Id`, `Enemy.AlertState`). No global `Enums.gd` god-class — see decision rubric below.
...
`_static_init` runs first time class referenced. `RegistryRoot._ready` references each registry once (e.g. `var _n: int = ItemRegistry.ALL.size()`) to force static-init order at boot — predictable, fail-loud, no `validate()` API on registry itself.
...
`class_name EnemyManager` here assumes Manager is **scene-root node** (instanced in a scene, where `class_name` correct + gives the type). Register in `[autoload]` instead → **drop `class_name`** (autoload name collides — see autoloads note above), access by autoload name.
...
`scenes/items/_pickup_base.tscn` root: `StaticBody3D` + `item_pickup.gd` script + collision_layer=`LAYER_INTERACTABLE` (player's interact raycast hits it) / mask=0 (pickup doesn't actively scan) + placeholder MeshInstance3D + CollisionShape3D children. Derived (`sword.tscn`, `potion.tscn`) `instance=ExtResource(_pickup_base)`, override only `item` (Def ref), `mesh`, `shape`.
...
1. `RegistryRoot` — forces every per-domain registry's `_static_init` by referencing it in `_ready`. Registries self-validate in `_static_init`, crash loud on failure.
2. `SaveSystem` — depends on registries (deserialize by ID).
3. `SoundBus` — pure pub-sub, no deps.

`WorldConstants` **not** autoloaded — `class_name`'d static-only RefCounted, accessed globally without `[autoload]` entry. See D9.

Boot-fail loud: `push_error` + `OS.crash` if registry can't validate — never let half-loaded registry hide behind null returns. `_static_init` path makes this automatic: moment `RegistryRoot._ready` touches a broken registry, crash fires before any gameplay code runs.

## When to break the skeleton

Skeleton = *default*, not prescription. Break when:
...
Three similar lines beats premature abstraction (base anti-overengineering rule). Don't add containers / autoloads / managers speculatively — wait for second instance.