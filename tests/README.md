...
| `bench_stringname.gd` | `bible/stringname-vs-string.md` | StringName vs String for `==` and dict-key lookup (~1.2× each) + the one-time interning cost. Shows the win is modest — mostly correctness + engine-API contract, not raw speed |
...
| `example_dod_save_proj/` | bible `dod-save-example.md` | runnable worked example — **save/load**: relational POD record (D4), ids-not-objects (D3), sparse door delta (D2), pure `to_record`/`from_record` transforms (D6), atomic write (`.tmp`+rename), safe `get_var(false)`, version-defaults migration. **Import first**, then `--path` — prints round-trip + v1 migration |
| `example_dod_upgrade_proj/` | bible `dod-upgrade-example.md` | runnable worked example — **stat/upgrade**: POD `WeaponDef`/`WeaponPartDef` (D1), pure `apply_part` flat→increased→more transform (D6), and the **authoring-equivalence test** (`apply_part(base,grip)` == hand-authored `sword_mk2.tres`, drift fails loud). **Import first**, then `--path` — prints PASS |
