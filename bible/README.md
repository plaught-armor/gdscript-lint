...
**Worked examples** (the rules composed, runnable + verified): [combat](dod-by-example.md) — enemy-combat subsystem, fat-`Enemy` strawman → data-oriented decomposition (D1–D8 + P6 + C2a); [perception](dod-perception-example.md) — existence-based alert state + the corrected D8 (inline SoA, do-less); [inventory](dod-inventory-example.md) — D11, one registry table with convention-derived assets, no mirror arrays; and [object pool](dod-pool-example.md) — P21 free-list + slot reuse, no per-frame alloc. Backed by `tests/example_dod_*_proj/` and literal runs on 4.8.dev.
