...
- **C1** — `const Packed*Array` reported byte-count size and read 0.0. [#88753](https://github.com/godotengine/godot/issues/88753). **Not reproduced on 4.6.stable or 4.8.dev** (size correct on both, `PackedStringArray`/`PackedFloat32Array`/`Array[PackedFloat32Array]` alike; the nested-constructor form is now a parse error) — `tests/repro_const_packed_shared.gd`. **Keep using `var`/`static var`, never `const`, on every version**, and not for the tracker symptom: a `const Packed*Array` is not read-only, and offers no `is_read_only()`/`make_read_only()` to ask. Bind it to a local, append, and the constant is rewritten process-wide with no error — a later independent binding reads the mutated value. That is what makes it different from C2's `Array`/`Dictionary`, which raise. If you need a shared table that is genuinely frozen, `const Array[T]` (or a `static var` + `make_read_only()`) is the container that gives it to you, and this is the one case where `Array[T]` beats the packed type outright (cf. S6). the S6b style point — init a `Packed*Array` with a bare literal, not a constructor wrapper — holds regardless. On an `@export` field that same wrapper is **S6c**, a data-loss bug rather than a style nit: it reads back null in the inspector and persists empty on save/reimport ([#106965](https://github.com/godotengine/godot/issues/106965)).
- **C2** — `const` `Array`/`Dictionary` was a shared mutable reference. [#61274](https://github.com/godotengine/godot/issues/61274). **Fixed as of 4.6.stable** (re-measured there and on 4.8.dev) — a `const` container is read-only; mutating it raises instead of silently corrupting the shared store. `Packed*Array` is the exception and is still silently shared and mutable — see C1. Discipline unchanged: never mutate a `const` container, `.duplicate()` first. (For class-shared `static var` tables the `.make_read_only()` lock — C2a above — is the live enforcement.)
...
|---|---|---|---|---|
| C1 | `const Packed*Array` corruption | [#88753](https://github.com/godotengine/godot/issues/88753) | **Live** | **Not reproduced** on 4.6/4.8 (size correct; nested-ctor form now a parse error) — but a const packed array is still **not read-only**, mutable through any binding |
| C2 | `const` arrays/dicts shared & mutable | [#61274](https://github.com/godotengine/godot/issues/61274) | **Live** (partial) | **Fixed** on 4.6/4.8 — `const` is read-only, mutation raises (`Packed*Array` excepted, C1) |
